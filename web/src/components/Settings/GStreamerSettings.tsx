...
const formatDecimalInput = (value: number | null | undefined) => {
...
const emptyConfig: GStreamerConfig = {
...
const componentStatusKind = (component?: GstComponentStatus | null) => {
...
const GStreamerSettings = forwardRef<GStreamerSettingsHandle>(function GStreamerSettings(_props, ref) {
  ...
  const [defaults, setDefaults] = useState<GStreamerConfig>(emptyConfig)
  const [status, setStatus] = useState<{ message: string; type: string }>({ message: '', type: '' })
  const [loading, setLoading] = useState(false)
  const [echoStatus, setEchoStatus] = useState<GstEchoStatus | null>(null)
  ...
  const updateField = (field: keyof GStreamerConfig, value: GStreamerConfig[keyof GStreamerConfig]) => {
    setGstreamerSettings(prev => ({ ...prev, [field]: value }))
  }

  const normalizeVersion = (version: string | number | null | undefined) => {
  ...
  const saveSettings = useCallback(async () => {
    ...
    try {
      ...
      setGstreamerSettings(config)
      queryClient.invalidateQueries({ queryKey: [GST_RUNTIME_QUERY_KEY] })
      setStatus({
    ...
    <GstSettingsContent>
      ...
      <FormGroup>
        <FormControlLabel
          ...
          labelPlacement='start'
        />
        <FormHelperText>{t('GStreamer.HDRToSDRHint')}</FormHelperText>
      </FormGroup>
      ...
      <FormGroup>
        <FormControlLabel
          ...
          labelPlacement='start'
        />
        <FormHelperText>{t('GStreamer.SubtitlesHint')}</FormHelperText>
      </FormGroup>
      ...
      <FormGroup>
        <FormControlLabel
          ...
          labelPlacement='start'
        />
        <FormHelperText>{t('GStreamer.UseGPUHint')}</FormHelperText>
      </FormGroup>
      ...
      <FormGroup>
        <FormControlLabel
          ...
          labelPlacement='start'
        />
        <FormHelperText>{t('GStreamer.HardwareAccelerationHint')}</FormHelperText>
      </FormGroup>
