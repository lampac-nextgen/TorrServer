...
import { useCreateCacheMap, SNAKE_MAX_CELLS } from '../customHooks'
...
const TorrentCache = ({ cache, isMini, isSnakeDebugMode }: TorrentCacheProps) => {
  ...
  // Mini needs fewer cells; detailed is capped so huge torrents don't freeze the dialog.
  const maxCells = isMini ? Math.min(SNAKE_MAX_CELLS, 1200) : SNAKE_MAX_CELLS
  const cacheMap = useCreateCacheMap(cache, maxCells)
  ...
  // Mini: responsive metrics. Detailed: fixed sizes like master (stable grid, less churn).
  const { pieceSize, gap } = useMemo(() => {
    if (isMini) return resolvePieceMetrics(baseSettings, width, true)
    return { pieceSize: baseSettings.pieceSize, gap: baseSettings.gapBetweenPieces }
  ...
  useEffect(() => {
    ...
    drawRaf.current = requestAnimationFrame(() => {
      ...
      for (let i = 0; i < drawSource.length; i++) {
        const { percentage = 0, priority = 0, isReader, isReaderRange } = drawSource[i]
...
/** Same gate as master: skip React work when Pieces/Readers unchanged. */
export default memo(
  TorrentCache,
  (prev, next) =>
